...
class Classification_Eth_6hr_No_Context_Dataset(Dataset):
    ...
    def __init__(self, mode):
        if mode == "train":
            ...
            self.data_path = constants.get_dataset_train_filepath(
                constants.CLASSIFICATION_ETH_6HR_NO_CONTEXT_TASK)
            self.label_path = constants.get_dataset_train_labelpath(
                constants.CLASSIFICATION_ETH_6HR_NO_CONTEXT_TASK)
        elif mode == "val":
            print(f"Setting up val Eth+6hr no context dataset...")
            self.data_path = constants.get_dataset_val_filepath(
                constants.CLASSIFICATION_ETH_6HR_NO_CONTEXT_TASK)
            self.label_path = constants.get_dataset_val_labelpath(
                constants.CLASSIFICATION_ETH_6HR_NO_CONTEXT_TASK)
        else:
            print(f"Error: mode should be one of [train, val] but got {mode} instead.")
...
class Classification_Eth_6hr_Dataset(Dataset):
    ...
    def __init__(self, mode):
        if mode == "train":
            print(f"Setting up train HDW no context dataset...")
            self.data_path = constants.get_dataset_train_filepath(constants.CLASSIFICATION_ETH_6HR_TASK)
            self.label_path = constants.get_dataset_train_labelpath(constants.CLASSIFICATION_ETH_6HR_TASK)
        elif mode == "val":
            print(f"Setting up val HDW no context dataset...")
            self.data_path = constants.get_dataset_val_filepath(constants.CLASSIFICATION_ETH_6HR_TASK)
            self.label_path = constants.get_dataset_val_labelpath(constants.CLASSIFICATION_ETH_6HR_TASK)
        else:
            print(f"Error: mode should be one of [train, val] but got {mode} instead.")
